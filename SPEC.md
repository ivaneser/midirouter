...
Режим автоматически обнаруживает связь между контроллером и синтезатором:

```
Phase 1: ОБНАРУЖЕНИЕ
┌─────────────────────────────────────────────┐
│  Контроллер → Pi: NoteOn [channel, note]    │
│  Но input порт не привязан к никакому output │
│  → Переход в режим WAITING_FOR_CC            │
│  → Клиент показывает: "Покрутите ручку на   │
│     синтезаторе для автоматической          │
│     настройки маршрута"                      │
└─────────────────────────────────────────────┘

Phase 2: ОБУЧЕНИЕ
┌─────────────────────────────────────────────┐
│  Синтезатор → Pi: CC #74 value=64           │
│  (например, крутите Cutoff на NTS-1)         │
│  → Роутер связывает input_0 → output_0      │
│  → Создаёт маршрут по умолчанию             │
│  → Клиент обновляет схему маршрутизации      │
└─────────────────────────────────────────────┘

Phase 3: РАБОТА
┌─────────────────────────────────────────────┐
│  Теперь все события с контроллера            │
│  автоматически идут на синтезатор           │
│  → Визуальная схема показывает связь         │
...
1. **Timeout обучения**: 30 секунд ожидания CC-сигнала. Если не пришло — режим сбрасывается.
2. **Многоканальность**: если контроллер на канале 3, а синтезатор получает CC на канале 5 — роутер понимает что это тот же синтезатор (CC от него).
3. **Несколько устройств**: если к Pi подключены NTS-1 и Craft Synth, и оба шлют CC одновременно — роутер создаёт два маршрута параллельно.
4. **Подтверждение на клиенте**: после обнаружения клиент показывает: "Обнаружена связь Akai LPK25 → Korg NTS-1. Подтвердить?" (можно отменить).
