...
1. **Multi-controller**: Все unrouted inputs становятся контроллерами, каждый подключается ко всем unrouted outputs.
2. **Ping timeout**: 5 тестовых нот с интервалом 1 сек; после таймаута — переход к следующему синтезатору (не зацикливание).
3. **Fallback discovery**: Если устройство шлёт MIDI без маршрута — через 5 секунд создаётся маршрут по умолчанию на первый доступный output порт.
4. **Защита от дублирования**: Флаги `discoveryActive` в сервере и воркере предотвращают повторные запуски auto-connect.
