...
### 6. Панели синтезаторов (Synth Controls)
Во вкладке «🔊 MIDI Devices» поддерживаемые выходы (Craft Synth, Korg NTS-1) отображаются карточками. Кнопка *Show controls* разворачивает параметры синтезатора (knobs/sliders из `device_maps/*.json`); перетаскивание отправляет MIDI CC на выбранный порт в реальном времени.
...
- **Пресеты конфигурации:** сохранение/загрузка настроек роутинга (`device_maps/daw_*.json`, вкладка ⚙️ Configuration).
