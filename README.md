...
2. Нажми паду ещё раз или переключи режим записи:
   ...
   - **Replace / Overdub** — первое нажатие пады начинает запись, повторное завершает запись и запускает клип. Ноты, которые ты играешь, запишутся в клип.
...
- **Мигающий cyan** во время воспроизведения — синхронизирован с MIDI clock, один период на долю.
- **Все пады гасятся** перед входом в DAW режим и при остановке клипов.

LED-команды отправляются на выход, указанный в профиле, не на синтезаторы. Для Launchkey channel 1 задаёт постоянный цвет, channel 2 — мигающий; цвет выбирается velocity в Note On.
